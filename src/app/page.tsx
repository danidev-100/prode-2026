...
export default async function HomePage() {
  ...
  const groupsFinished = groups.filter((g) => {
    const groupMatches = matches.filter((m) => m.group === g && m.stage === "GROUP")
  ...
  const now = new Date()
  const daysLeft = Math.max(0, Math.ceil((tournamentStart.getTime() - now.getTime()) / 86400000))
      ...
      <div className="relative mb-8 sm:mb-10">
        ...
        <div className="relative">
          ...
          <p className="text-text-secondary mt-2 sm:mt-3 text-sm sm:text-base max-w-lg leading-relaxed">
            ...
            <span className="text-text-primary font-medium">Canadá 🇨🇦 México 🇲🇽 EE.UU. 🇺🇸</span>
          </p>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 sm:mt-5">
            <div className="flex items-center gap-1.5 text-xs sm:text-sm">
              <div className="flex items-center gap-1 bg-bg-secondary border border-border rounded-lg px-2.5 py-1.5">
                ...
                <span className="text-text-primary font-semibold tabular-nums">{finishedCount}</span>
                <span className="text-text-muted">/ {totalMatches} jugados</span>
              </div>
            </div>
            {groupsFinished > 0 && (
              <div className="flex items-center gap-1 text-xs sm:text-sm">
                <div className="flex items-center gap-1 bg-bg-secondary border border-border rounded-lg px-2.5 py-1.5">
                  ...
                  <span className="text-text-primary font-semibold tabular-nums">{groupsFinished}</span>
            ...
            {daysLeft > 0 && (
              <div className="flex items-center gap-1 text-xs sm:text-sm animate-shimmer">
                <div className="flex items-center gap-1 bg-gold/5 border border-gold/20 rounded-lg px-2.5 py-1.5">
                  ...
                  <span className="text-gold font-semibold tabular-nums">{daysLeft}</span>
