...
export default async function RankingPage() {
	...
	return (
		<>
			{/* Dibu background — right side, fixed, 22% opacity, faded */}
			<div
				className="fixed top-0 left-0 pointer-events-none z-0"
				style={{
					width: "100vw",
					height: "100dvh",
					backgroundImage: "url('/dibu - copia.webp')",
					...
					<>
						...
						{ranked.length >= 3 && (
							<div className="hidden sm:flex items-end justify-center gap-4 mb-10 h-44">
								...
								<div className="flex flex-col items-center w-28">
									...
									<div
										className="w-full bg-bg-tertiary/10 border border-silver/20 rounded-t-lg mt-2 flex items-center justify-center"
										style={{ height: "80px" }}
									>
								...
								<div className="flex flex-col items-center w-28">
									...
									<div
										className="w-full bg-bg-tertiary/10 border border-bronze/20 rounded-t-lg mt-2 flex items-center justify-center"
										style={{ height: "64px" }}
									>
							...
							<div className="sm:hidden grid grid-cols-3 gap-2 mb-6">
								{[ranked[1], ranked[0], ranked[2]].map((user, i) => {
									...
									return (
										<div
											key={user.id}
											className={`bg-transparent border ${borders[pos]} rounded-xl p-3 text-center
                      ${pos === 1 ? "bg-gradient-to-b from-gold/10 to-transparent" : ""}`}
										>
											...
											<div
												className={`text-lg font-bold tabular-nums mt-1 ${texts[pos]}`}
											>
												{user.points}
											</div>
